...
export class CustomError extends HttpException {
   ...
   * @param data Body de la respuesta del error
   */
  constructor(errorType?: TypeEnumError) {
    const baseError = getErrorDetails(errorType ?? EnumError500.ErrorDesconocido)

    super(baseError.message, baseError.statusCode)

    this.name = "CustomError"
    this.code = errorType ?? EnumError500.ErrorDesconocido
    this.base = baseError
